...
void add_common_parameters_and_datum(PartDocument& document) {
  ...
  REQUIRE(document.add_parameter(
      make_length_parameter("part.hole_diameter", "hole_diameter", 12.0)));
...
PartDocument make_pattern_document() {
  ...
  REQUIRE(document.value().add_parameter(
      make_count_parameter("part.pattern_count", "pattern_count", 6.0)));
...
PartDocument make_ambiguous_circle_document() {
  ...
  REQUIRE(sketch);
  auto first = CircleProfile::create(ProfileId("profile.hole.a"),
                                     ParameterId("part.hole_diameter"), Point2{-10.0, 0.0});
  auto second = CircleProfile::create(ProfileId("profile.hole.b"),
                                      ParameterId("part.hole_diameter"), Point2{10.0, 0.0});
  REQUIRE(first);
  REQUIRE(second);
...
TEST_CASE("Generated topology producer matrices publish finite unique cardinality-one roles",
  ...
  for (const auto& rule : rectangular) {
    CHECK(std::count_if(rectangular.begin(), rectangular.end(), [&](const auto& candidate) {
            return candidate.family == rule.family && candidate.role == rule.role;
          }) == 1);
  }
  ...
  const GeneratedTopologyRoleRule expected_wall{
      GeneratedTopologyReferenceFamily::CylindricalFace, "wall", 1U};
  const GeneratedTopologyRoleRule expected_source{
      GeneratedTopologyReferenceFamily::CircularEdge, "source_rim", 1U};
  const GeneratedTopologyRoleRule expected_opposite{
      GeneratedTopologyReferenceFamily::CircularEdge, "opposite_rim", 1U};
  CHECK(circular[0] == expected_wall);
  CHECK(circular[1] == expected_source);
  CHECK(circular[2] == expected_opposite);
}
...
TEST_CASE("Supported generated topology references validate from producer intent",
  ...
  auto linear = validate_generated_topology_reference(document, linear_edge_reference());
  REQUIRE(linear);
  CHECK(linear.value() == 1U);
...
TEST_CASE("Generated topology identity is independent of feature insertion order",
  ...
  const auto first_cylinder = cylinder_reference();
  const auto second_cylinder = cylinder_reference();
  const auto first_edge = linear_edge_reference(SemanticEdge::BackLeft);
  const auto second_edge = linear_edge_reference(SemanticEdge::BackLeft);

  REQUIRE(validate_generated_topology_reference(base_first, first_cylinder));
  REQUIRE(validate_generated_topology_reference(auxiliary_first, second_cylinder));
  REQUIRE(validate_generated_topology_reference(base_first, first_edge));
  REQUIRE(validate_generated_topology_reference(auxiliary_first, second_edge));
  CHECK(topology_spelling(first_cylinder) == topology_spelling(second_cylinder));
  CHECK(topology_spelling(first_edge) == topology_spelling(second_edge));
  CHECK(topology_spelling(first_cylinder) ==
        "topo:cylindrical_face:feature%2Ehole:profile%2Ehole:wall");
  CHECK(topology_spelling(first_edge) == "topo:linear_edge:feature%2Ebase:back_left");
...
TEST_CASE("Assembly JSON preserves generated topology semantic spellings byte-for-byte",
  ...
  auto target_a =
      AssemblyConstraintTarget::create(ComponentInstanceId("component.part"), cylindrical);
