...
PartDocument make_right_face_cut_document(Point2 hole_center = {}) {
  auto document = PartDocument::create(DocumentId("part.right_face_plate"), "RightFacePlate");
  REQUIRE(document);
...
ShapeCache make_shape_cache() {
  auto cache = ShapeCache::create(ShapeCacheId("shape_cache.right_face"));
  REQUIRE(cache);
...
TEST_CASE("Right-face workplane recomputes an axis-directed through-all circular cut",
  ...
  CHECK(cache.final_feature_id().value() == "feature.right_hole_cut");
...
TEST_CASE("Right-face workplane rejects holes outside side-face bounds", "[geometry][workplane]") {
  const PartDocument document = make_right_face_cut_document(Point2{0.0, 3.5});
  ShapeCache cache = make_shape_cache();
  const GeometryRecomputeExecutor executor;
