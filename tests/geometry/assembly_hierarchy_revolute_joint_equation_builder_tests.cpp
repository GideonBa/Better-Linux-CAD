...
AssemblyHierarchyJoint joint(AssemblyHierarchyConstraintEndpoint target_a,
  ...
  auto value = AssemblyHierarchyJoint::create(
      ...
      angle(coordinate, "joint.cross.revolute"));
  REQUIRE(value);
  return value.value();
}
...
Project root_child_project(RigidTransform boundary = identity_rigid_transform(),
  ...
  REQUIRE(root.value().add_subassembly_instance(
      occurrence("subassembly.left", boundary)));
...
TEST_CASE("Hierarchy Revolute equations reuse local directed axis seating and twist semantics",
          "[geometry][assembly-cross-hierarchy-revolute]") {
  const AssemblyHierarchyRevoluteJointEquationBuilder builder;

  SECTION("Root to child satisfied at zero") {
    ...
    auto equation = builder.build(project, value, angle(0.0, "joint.cross.revolute"));
    REQUIRE(equation);
    CHECK(equation.value().residual.direction_alignment.x == Approx(0.0).margin(1.0e-12));
    CHECK(equation.value().residual.direction_alignment.y == Approx(0.0).margin(1.0e-12));
    CHECK(equation.value().residual.direction_alignment.z == Approx(0.0).margin(1.0e-12));
    CHECK(equation.value().residual.axis_offset_mm.x == Approx(0.0).margin(1.0e-12));
    CHECK(equation.value().residual.axis_offset_mm.y == Approx(0.0).margin(1.0e-12));
    CHECK(equation.value().residual.axis_offset_mm.z == Approx(0.0).margin(1.0e-12));
  ...
  SECTION("Signed requested coordinate follows root-space orientation") {
    ...
    REQUIRE(negative);
    CHECK_FALSE(std::abs(negative.value().residual.twist_alignment_sine) < 1.0e-6);
  }
}
...
TEST_CASE("Repeated same-authority Revolute endpoints retain distinct root-space paths",
          "[geometry][assembly-cross-hierarchy-revolute]") {
...
TEST_CASE("Hierarchy Revolute equation construction fails closed on non-seat target semantics",
          "[geometry][assembly-cross-hierarchy-revolute]") {
  const Project project = root_child_project();
  const AssemblyHierarchyJoint value = joint(
      ...
      endpoint({"subassembly.left"}, "component.child", "feature.hole.seat"));
  const AssemblyHierarchyRevoluteJointEquationBuilder builder;
  CHECK(builder.build(project, value, angle(0.0, "joint.cross.revolute")).has_error());
}
