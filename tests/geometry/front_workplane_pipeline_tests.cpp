...
PartDocument make_front_face_cut_document(Point2 hole_center = {}) {
  auto document = PartDocument::create(DocumentId("part.front_face_plate"), "FrontFacePlate");
  REQUIRE(document);
  ...
  auto workplane = DerivedWorkplane::create_on_feature_face(
      DatumPlaneId("workplane.base_front"), "BaseFrontFace", face_reference.value());
  REQUIRE(workplane);
  REQUIRE(document.value().add_derived_workplane(workplane.value()));

  auto side_hole_sketch = Sketch::create(SketchId("sketch.front_hole"), "Sketch_FrontHole",
                                         DatumPlaneId("workplane.base_front"));
  REQUIRE(side_hole_sketch);
  auto circle = CircleProfile::create(ProfileId("profile.front_hole"),
  ...
  auto cut = Feature::create_subtractive_extrude(FeatureId("feature.front_hole_cut"),
                                                 "FrontHoleCut", SketchId("sketch.front_hole"),
...
ShapeCache make_shape_cache() {
  auto cache = ShapeCache::create(ShapeCacheId("shape_cache.front_face"));
  REQUIRE(cache);
...
TEST_CASE("Front-face workplane recomputes an axis-directed through-all circular cut",
          "[geometry][workplane]") {
  const PartDocument document = make_front_face_cut_document(Point2{-12.0, 0.0});
  ...
  CHECK(cache.final_feature_id().value() == "feature.front_hole_cut");
...
TEST_CASE("Front-face workplane rejects holes outside side-face bounds", "[geometry][workplane]") {
  const PartDocument document = make_front_face_cut_document(Point2{0.0, 3.5});
  ShapeCache cache = make_shape_cache();
  const GeometryRecomputeExecutor executor;
