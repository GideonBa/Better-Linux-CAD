...
PartDocument make_left_face_cut_document(Point2 hole_center = {}) {
  auto document = PartDocument::create(DocumentId("part.left_face_plate"), "LeftFacePlate");
  REQUIRE(document);
  ...
  auto workplane = DerivedWorkplane::create_on_feature_face(DatumPlaneId("workplane.base_left"),
                                                            "BaseLeftFace", face_reference.value());
  REQUIRE(workplane);
  REQUIRE(document.value().add_derived_workplane(workplane.value()));

  auto side_hole_sketch = Sketch::create(SketchId("sketch.left_hole"), "Sketch_LeftHole",
                                         DatumPlaneId("workplane.base_left"));
  REQUIRE(side_hole_sketch);
  auto circle = CircleProfile::create(ProfileId("profile.left_hole"),
  ...
  auto cut = Feature::create_subtractive_extrude(FeatureId("feature.left_hole_cut"), "LeftHoleCut",
                                                 SketchId("sketch.left_hole"),
...
ShapeCache make_shape_cache() {
  auto cache = ShapeCache::create(ShapeCacheId("shape_cache.left_face"));
  REQUIRE(cache);
...
TEST_CASE("Left-face workplane recomputes an axis-directed through-all circular cut",
          "[geometry][workplane]") {
  const PartDocument document = make_left_face_cut_document(Point2{-12.0, 0.0});
  ...
  CHECK(cache.final_feature_id().value() == "feature.left_hole_cut");
...
TEST_CASE("Left-face workplane rejects holes outside side-face bounds", "[geometry][workplane]") {
  const PartDocument document = make_left_face_cut_document(Point2{0.0, 3.5});
  ShapeCache cache = make_shape_cache();
  const GeometryRecomputeExecutor executor;
  ...
  CHECK(summary.error().object_id() == "profile.left_hole");
  ...
  CHECK(cache.find_feature_shape(FeatureId("feature.left_hole_cut")) == nullptr);
}
