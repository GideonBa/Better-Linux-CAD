...
AssemblyJoint joint(const char* id, const char* component_a, const char* component_b) {
  auto value = AssemblyJoint::create(AssemblyJointId(id), id, AssemblyJointType::Revolute,
                                     target(component_a), target(component_b),
                                     AssemblyJointState::Active, angle(-90.0, id),
...
AssemblyJointMotionInputSnapshot snapshot(const AssemblyJoint& value) {
  return AssemblyJointMotionInputSnapshot{value.id(), value.type(), value.state(), value.target_a(),
                                          value.target_b(), value.limits(),
                                          value.coordinate_deg()};
}
...
Project make_project() {
  ...
  auto project = Project::create(DocumentId("project.snapshot"), "SnapshotProject", assembly.value());
  REQUIRE(project);
