...
enum class SketchReferenceTargetKind {
  LineSegment,
  LineSegmentStart,
  LineSegmentEnd,
  ProjectedPoint,
...
enum class SketchConstraintKind {
  CoincidentToProjectedPoint,
  ParallelToProjectedLine,
  CollinearWithProjectedLine
};
...
public:
  ...
  [[nodiscard]] static Result<SketchReferenceTarget>
  create_line_segment_start(SketchEntityId entity);
...
public:
  [[nodiscard]] static Result<SketchConstraint>
  create_coincident_to_projected_point(SketchConstraintId id,
                                       SketchReferenceTarget constrained_point,
                                       SketchReferenceTarget projected_point);
  [[nodiscard]] static Result<SketchConstraint>
  create_parallel_to_projected_line(SketchConstraintId id, SketchReferenceTarget constrained_line,
                                    SketchReferenceTarget projected_line);
  [[nodiscard]] static Result<SketchConstraint>
  create_collinear_with_projected_line(SketchConstraintId id,
...
private:
  SketchConstraint(SketchConstraintId id, SketchConstraintKind kind,
...
public:
  [[nodiscard]] static Result<SketchGeometricConstraint> create_fixed(SketchConstraintId id,
                                                                      SketchReferenceTarget target);
  [[nodiscard]] static Result<SketchGeometricConstraint>
  create_horizontal(SketchConstraintId id, SketchReferenceTarget line);
  [[nodiscard]] static Result<SketchGeometricConstraint>
  create_vertical(SketchConstraintId id, SketchReferenceTarget line);
  [[nodiscard]] static Result<SketchGeometricConstraint>
  create_parallel(SketchConstraintId id, SketchReferenceTarget first_line,
                  SketchReferenceTarget second_line);
  [[nodiscard]] static Result<SketchGeometricConstraint>
  create_perpendicular(SketchConstraintId id, SketchReferenceTarget first_line,
                       SketchReferenceTarget second_line);
  [[nodiscard]] static Result<SketchGeometricConstraint>
  create_equal_length(SketchConstraintId id, SketchReferenceTarget first_line,
                      SketchReferenceTarget second_line);
  [[nodiscard]] const SketchConstraintId& id() const noexcept;
...
public:
  [[nodiscard]] static Result<SketchDrivingDimension>
  create_horizontal_distance(SketchDimensionId id, SketchReferenceTarget first_point,
                             SketchReferenceTarget second_point, ParameterId parameter);
  [[nodiscard]] static Result<SketchDrivingDimension>
  create_vertical_distance(SketchDimensionId id, SketchReferenceTarget first_point,
                           SketchReferenceTarget second_point, ParameterId parameter);
  [[nodiscard]] static Result<SketchDrivingDimension>
  create_aligned_distance(SketchDimensionId id, SketchReferenceTarget first_point,
                          SketchReferenceTarget second_point, ParameterId parameter);
  [[nodiscard]] static Result<SketchDrivingDimension>
  create_point_to_point_distance(SketchDimensionId id, SketchReferenceTarget first_point,
                                 SketchReferenceTarget second_point, ParameterId parameter);
...
public:
  [[nodiscard]] static Result<ReferenceGeneratedLine>
  create_from_projected_point_constraints(SketchEntityId id, SketchConstraintId start_constraint,
                                          SketchConstraintId end_constraint);
  [[nodiscard]] static Result<ReferenceGeneratedLine>
  create_with_projected_line_direction(SketchEntityId id, SketchConstraintId start_constraint,
                                       SketchConstraintId end_constraint,
...
class SplineSegment {
public:
  [[nodiscard]] static Result<SplineSegment> create_cubic_bezier(SketchEntityId id, Point2 start,
...
public:
  [[nodiscard]] static Result<SketchTrimExtendOperation>
  create_trim(SketchTrimOperationId id, SketchEntityId target_entity, Point2 replacement_endpoint);
  [[nodiscard]] static Result<SketchTrimExtendOperation> create_extend(SketchTrimOperationId id,
...
public:
  [[nodiscard]] static Result<SketchTangentContinuity>
  create_tangent(SketchConstraintId id, SketchEntityId first_entity, SketchEntityId second_entity);
  [[nodiscard]] const SketchConstraintId& id() const noexcept;
...
private:
  SketchTangentContinuity(SketchConstraintId id, SketchTangentContinuityKind kind,
                          SketchEntityId first_entity, SketchEntityId second_entity);
  SketchConstraintId id_;
...
public:
  [[nodiscard]] static Result<CompositeClosedProfile>
  create(ProfileId id, std::vector<SketchEntityId> outer_contour,
...
// A parametric bolt circle: `count` equally spaced holes of `hole_diameter`
// on a circle of `radius` around `center`. The pattern is stored as model
// intent; the geometry layer expands it into individual cuts.
class CircularHolePattern {
public:
  [[nodiscard]] static Result<CircularHolePattern>
  create(ProfileId id, ParameterId radius_parameter, ParameterId count_parameter,
         ParameterId hole_diameter_parameter, Point2 center = {}, double angle_offset_deg = 0.0);
  [[nodiscard]] const ProfileId& id() const noexcept;
  [[nodiscard]] Point2 center() const noexcept;
  [[nodiscard]] double angle_offset_deg() const noexcept;
  [[nodiscard]] const ParameterId& radius_parameter() const noexcept;
  [[nodiscard]] const ParameterId& count_parameter() const noexcept;
  [[nodiscard]] const ParameterId& hole_diameter_parameter() const noexcept;

private:
  CircularHolePattern(ProfileId id, ParameterId radius_parameter, ParameterId count_parameter,
                      ParameterId hole_diameter_parameter, Point2 center, double angle_offset_deg);
  ProfileId id_;
  ParameterId radius_parameter_;
  ParameterId count_parameter_;
  ParameterId hole_diameter_parameter_;
  Point2 center_;
...
public:
  ...
  [[nodiscard]] Result<std::size_t> add_profile(CircularHolePattern pattern);
  ...
  reference_generated_lines() const noexcept;
  ...
  geometric_constraints() const noexcept;
  [[nodiscard]] const std::vector<SketchDrivingDimension>& driving_dimensions() const noexcept;
  [[nodiscard]] const std::vector<SketchTrimExtendOperation>&
  trim_extend_operations() const noexcept;
  ...
  [[nodiscard]] const std::vector<CompositeClosedProfile>&
  composite_closed_profiles() const noexcept;
  [[nodiscard]] const std::vector<CircularHolePattern>& circular_hole_patterns() const noexcept;
  ...
  find_reference_generated_line(SketchEntityId id) const noexcept;
  ...
  find_geometric_constraint(SketchConstraintId id) const noexcept;
  [[nodiscard]] const SketchDrivingDimension*
  find_driving_dimension(SketchDimensionId id) const noexcept;
  [[nodiscard]] const SketchTrimExtendOperation*
  find_trim_extend_operation(SketchTrimOperationId id) const noexcept;
  [[nodiscard]] const SketchTangentContinuity*
  find_tangent_continuity(SketchConstraintId id) const noexcept;
  ...
  [[nodiscard]] const CompositeClosedProfile*
  find_composite_closed_profile(ProfileId id) const noexcept;
  [[nodiscard]] const CircularHolePattern* find_circular_hole_pattern(ProfileId id) const noexcept;
  [[nodiscard]] Result<std::vector<Point2>>
  closed_profile_vertices(const ClosedProfile& profile) const;
  [[nodiscard]] Result<std::vector<Point2>>
  arc_closed_profile_vertices(const ArcClosedProfile& profile) const;
...
private:
  ...
  [[nodiscard]] Result<std::size_t> validate_constraint_target(const SketchReferenceTarget& target,
                                                               const std::string& object_id) const;
  [[nodiscard]] Result<std::size_t> validate_point_target(const SketchReferenceTarget& target,
                                                          const std::string& object_id) const;
  [[nodiscard]] Result<std::size_t>
  validate_explicit_line_target(const SketchReferenceTarget& target,
                                const std::string& object_id) const;
  [[nodiscard]] Result<std::vector<Point2>>
  validate_closed_profile(const ClosedProfile& profile) const;
  [[nodiscard]] Result<std::vector<Point2>>
  validate_arc_closed_profile(const ArcClosedProfile& profile) const;
  [[nodiscard]] Result<std::size_t>
  validate_composite_closed_profile(const CompositeClosedProfile& profile) const;
  SketchId id_;
  std::string name_;
  ...
  std::vector<SketchTangentContinuity> tangent_continuities_;
  ...
  std::vector<CircularHolePattern> circular_hole_patterns_;
};
