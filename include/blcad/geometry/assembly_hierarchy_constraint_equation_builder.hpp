...
namespace assembly_hierarchy_constraint_detail {
...
[[nodiscard]] inline Error validation_error(std::string object_id, std::string message) {
  return Error::validation(std::move(object_id), std::move(message));
}

[[nodiscard]] inline std::string occurrence_object_id(
...
[[nodiscard]] inline Result<ResolvedOccurrenceContext> resolve_occurrence(
...
[[nodiscard]] inline Result<Project> make_local_target_view(const Project& project,
...
[[nodiscard]] inline Result<AssemblyConstraintTarget> make_local_target(
...
[[nodiscard]] inline std::vector<RigidTransform> target_transform_chain(
    const RigidTransform& component_transform,
...
[[nodiscard]] inline AssemblySpacePlanarDescriptor evaluate_plane(
    const std::vector<RigidTransform>& transforms, const ComponentLocalPlanarDescriptor& local) {
...
[[nodiscard]] inline AssemblySpaceAxisDescriptor evaluate_axis(
    const std::vector<RigidTransform>& transforms, const ComponentLocalAxisDescriptor& local) {
  const AssemblyHierarchyTransformEvaluator evaluator;
...
[[nodiscard]] inline Vector3 difference(const Point3& target, const Point3& source) noexcept {
  return Vector3{target.x - source.x, target.y - source.y, target.z - source.z};
}

[[nodiscard]] inline Vector3 add(const Vector3& lhs, const Vector3& rhs) noexcept {
  return Vector3{lhs.x + rhs.x, lhs.y + rhs.y, lhs.z + rhs.z};
}

[[nodiscard]] inline double dot(const Vector3& lhs, const Vector3& rhs) noexcept {
...
inline Result<AssemblyHierarchyConstraintTarget> AssemblyHierarchyConstraintTarget::create(
    std::vector<SubassemblyInstanceId> occurrence_path,
    ComponentInstanceId component_instance,
    std::string semantic_reference) {
  ...
  auto local_target =
      AssemblyConstraintTarget::create(component_instance, semantic_reference);
...
inline AssemblyHierarchyConstraintTarget::AssemblyHierarchyConstraintTarget(
    std::vector<SubassemblyInstanceId> occurrence_path,
    ComponentInstanceId component_instance,
    std::string semantic_reference)
...
inline const std::vector<SubassemblyInstanceId>&
AssemblyHierarchyConstraintTarget::occurrence_path() const noexcept {
  return occurrence_path_;
}

inline const ComponentInstanceId&
AssemblyHierarchyConstraintTarget::component_instance() const noexcept {
  return component_instance_;
}

inline const std::string& AssemblyHierarchyConstraintTarget::semantic_reference() const noexcept {
  return semantic_reference_;
}

inline Result<AssemblyHierarchyConstraintQuery> AssemblyHierarchyConstraintQuery::create(
  ...
  auto validated = AssemblyConstraint::create(id, validation_name, type, std::move(local_target_a.value()),
                                              std::move(local_target_b.value()),
                                              AssemblyConstraintState::Active, distance, angle);
...
inline AssemblyHierarchyConstraintQuery::AssemblyHierarchyConstraintQuery(
    AssemblyConstraintId id,
    AssemblyConstraintType type,
    AssemblyHierarchyConstraintTarget target_a,
    AssemblyHierarchyConstraintTarget target_b,
...
inline const AssemblyConstraintId& AssemblyHierarchyConstraintQuery::id() const noexcept {
  return id_;
}

inline AssemblyConstraintType AssemblyHierarchyConstraintQuery::type() const noexcept {
  return type_;
}

inline const AssemblyHierarchyConstraintTarget&
...
inline const std::optional<Quantity>& AssemblyHierarchyConstraintQuery::distance() const noexcept {
  return distance_;
}

inline const std::optional<Quantity>& AssemblyHierarchyConstraintQuery::angle() const noexcept {
  return angle_;
}

inline Result<AssemblyHierarchyPlanarConstraintTargetDescriptor>
AssemblyHierarchyConstraintTargetResolver::resolve_planar(
  ...
  using namespace assembly_hierarchy_constraint_detail;
  ...
  const auto transforms = target_transform_chain(resolved.value().component_transform,
...
inline Result<AssemblyHierarchyAxisConstraintTargetDescriptor>
...
inline Result<AssemblyHierarchyInsertConstraintTargetDescriptor>
...
inline Result<AssemblyHierarchyConstraintEquationDescriptor>
AssemblyHierarchyConstraintEquationBuilder::build(
  ...
  if (query.type() == AssemblyConstraintType::Concentric) {
    ...
    const Vector3 origin_delta = difference(target_b.value().axis.origin, target_a.value().axis.origin);
  ...
  const Vector3 origin_delta = difference(target_b.value().plane.origin, target_a.value().plane.origin);
