...
[[nodiscard]] bool same_point(Point3 left, Point3 right) noexcept {
  return std::abs(left.x - right.x) <= kAxisTolerance &&
         std::abs(left.y - right.y) <= kAxisTolerance &&
         std::abs(left.z - right.z) <= kAxisTolerance;
}
...
[[nodiscard]] ClosedProfileCurveSegment translate_curve_along_axis(ClosedProfileCurveSegment curve,
  ...
  curve.control2 = translate_along_axis(curve.control2, axis, delta);
  return curve;
}
  ...
  if (curve.kind == ClosedProfileCurveKind::CubicBezierSpline) {
    if (same_point(curve.start, curve.end)) {
      return Result<TopoDS_Edge>::failure(
          make_geometry_error("spline curve edge endpoints must be distinct"));
    }
    ...
    if (!edge_builder.IsDone()) {
      return Result<TopoDS_Edge>::failure(make_geometry_error("could not build spline curve edge"));
    }
    return Result<TopoDS_Edge>::success(edge_builder.Edge());
  }
