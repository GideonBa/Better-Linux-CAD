...
[[nodiscard]] Result<std::vector<LocalLine>> collect_local_lines(const PartDocument& document,
  ...
  for (const auto& line : sketch.line_segments()) {
    ...
    if (resolved.has_error())
      return Result<std::vector<LocalLine>>::failure(resolved.error());
  ...
  for (const auto& line : sketch.reference_generated_lines()) {
    ...
    if (resolved.has_error())
      return Result<std::vector<LocalLine>>::failure(resolved.error());
    lines.push_back(
        LocalLine{resolved.value().id(), resolved.value().start(), resolved.value().end()});
  }
...
[[nodiscard]] std::size_t point_index(std::vector<Point2>& points, Point2 point) {
  for (std::size_t index = 0; index < points.size(); ++index) {
    if (same_point(points[index], point))
      return index;
  }
...
[[nodiscard]] Result<std::vector<LocalLine>> order_single_loop(std::vector<LocalLine> lines,
  ...
  for (std::size_t line_index = 0; line_index < lines.size(); ++line_index) {
    ...
    if (incident.size() < points.size())
      incident.resize(points.size());
  ...
  for (std::size_t i = 0; i < ordered.size(); ++i) {
    for (std::size_t j = i + 1U; j < ordered.size(); ++j) {
      if (are_adjacent_segments(i, j, ordered.size()))
        continue;
      if (segments_intersect(ordered[i].start, ordered[i].end, ordered[j].start, ordered[j].end)) {
        return Result<std::vector<LocalLine>>::failure(validation_error(
            object_id, "automatic region detection rejects self-intersecting regions"));
      }
    }
  }
...
Result<GeneratedClosedProfileCandidate>
SketchRegionFinder::find_single_region(const PartDocument& document, const Sketch& sketch) const {
  auto lines = collect_local_lines(document, sketch);
  if (lines.has_error())
    return Result<GeneratedClosedProfileCandidate>::failure(lines.error());

  auto ordered = order_single_loop(std::move(lines.value()), sketch.id().value());
  if (ordered.has_error())
    return Result<GeneratedClosedProfileCandidate>::failure(ordered.error());
...
Result<ClosedProfile>
SketchRegionFinder::make_closed_profile(const GeneratedClosedProfileCandidate& candidate) const {
