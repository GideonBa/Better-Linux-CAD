...
[[nodiscard]] std::vector<ComponentInstanceId> collect_combined_relationship_group(
    const AssemblyConstraintGraph& constraint_graph, const AssemblyJointGraph& joint_graph,
  ...
  std::sort(group.begin(), group.end(), [](const ComponentInstanceId& lhs,
...
AssemblyJointMotionSolver::move(const Project& project, AssemblyJointId joint_id,
  ...
  if (requested_coordinate.kind() != QuantityKind::AngleDeg) {
    return Result<AssemblyJointMotionResult>::failure(validation_error(
        joint_id.value(), "revolute joint motion coordinate must use degrees"));
  }
  ...
  if (requested_deg < joint->limits().lower_deg || requested_deg > joint->limits().upper_deg) {
    return Result<AssemblyJointMotionResult>::failure(validation_error(
  ...
  if (target_a == nullptr || target_b == nullptr) {
    return Result<AssemblyJointMotionResult>::failure(validation_error(
        joint_id.value(), "revolute joint motion target components must exist"));
  }
  if (target_a->suppression_state() == ComponentSuppressionState::Suppressed ||
    ...
    return Result<AssemblyJointMotionResult>::failure(validation_error(
        joint_id.value(), "revolute joint motion requires active non-suppressed target components"));
  }
  ...
  const std::vector<ComponentInstanceId> combined_group = collect_combined_relationship_group(
      constraint_graph.value(), joint_graph.value(), *joint);
...
AssemblyJointMotionResultApplier::apply(Project& project,
  ...
  auto coordinate_updated =
      candidate_project.assembly().set_joint_coordinate(result.joint, requested.value());
