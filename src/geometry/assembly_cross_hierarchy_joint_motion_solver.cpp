...
[[nodiscard]] AssemblyLocalRelationshipInputSnapshot
make_local_constraint_snapshot(const DocumentId& assembly_document,
                               const AssemblyConstraint& constraint) {
  return AssemblyLocalRelationshipInputSnapshot{assembly_document,
                                                constraint.id(),
                                                constraint.name(),
                                                constraint.type(),
                                                constraint.target_a(),
                                                constraint.target_b(),
                                                constraint.state(),
                                                distance_mm(constraint.distance()),
...
make_cross_constraint_snapshot(const AssemblyHierarchyConstraint& constraint) {
  return AssemblyProjectCrossHierarchyRelationshipInputSnapshot{constraint.id(),
                                                                constraint.name(),
                                                                constraint.type(),
                                                                constraint.target_a(),
                                                                constraint.target_b(),
...
[[nodiscard]] AssemblyLocalJointMotionInputSnapshot
make_local_joint_snapshot(const DocumentId& assembly_document, const AssemblyJoint& joint) {
  return AssemblyLocalJointMotionInputSnapshot{assembly_document,      joint.id(),
                                               joint.name(),           joint.type(),
                                               joint.target_a(),       joint.target_b(),
                                               joint.state(),          joint.limits(),
                                               joint.coordinate_deg(), joint.coordinate_slots()};
}

[[nodiscard]] AssemblyProjectCrossHierarchyJointMotionInputSnapshot
make_cross_joint_snapshot(const AssemblyHierarchyJoint& joint) {
  return AssemblyProjectCrossHierarchyJointMotionInputSnapshot{
      joint.id(),       joint.name(),           joint.type(),
      joint.target_a(), joint.target_b(),       joint.state(),
      joint.limits(),   joint.coordinate_deg(), joint.coordinate_slots()};
}
      ...
      [&](const auto& value) -> Result<AssemblyCrossHierarchyMotionRelationshipInputSnapshot> {
        ...
        if constexpr (std::is_same_v<Identity, AssemblyLocalRelationshipIdentity>) {
          const AssemblyDocument* assembly =
              project.find_assembly_document(value.assembly_document);
          if (assembly == nullptr) {
          ...
          if (joint == nullptr) {
            return Result<AssemblyCrossHierarchyMotionRelationshipInputSnapshot>::failure(
                validation_error(value.joint.value(),
                                 "cross-hierarchy motion local joint must exist"));
          }
          return Result<AssemblyCrossHierarchyMotionRelationshipInputSnapshot>::success(
...
validate_motion_relationship_freshness(const Project& project,
  ...
  if (result.relationship_snapshots.size() != result.relationships.size()) {
    return Result<std::size_t>::failure(
        validation_error("assembly.cross_hierarchy_motion_application",
                         "cross-hierarchy motion result relationship snapshot set is incomplete or "
                         "contains extras"));
  }
...
[[nodiscard]] Result<std::size_t> validate_motion_group_freshness(
  ...
  if (std::find(graph.value().motion_groups().begin(), graph.value().motion_groups().end(),
    ...
    return Result<std::size_t>::failure(
        validation_error("assembly.cross_hierarchy_motion_application",
                         "cross-hierarchy motion result is stale because active motion-group "
...
[[nodiscard]] Result<std::size_t>
validate_selected_joint_snapshot(const Project& project,
                                 const AssemblyCrossHierarchyJointMotionResult& result) {
  ...
  for (const auto& snapshot : result.relationship_snapshots) {
    ...
    if (selected != nullptr) {
      return Result<std::size_t>::failure(
          validation_error(result.joint.value(),
                           "cross-hierarchy motion result duplicates selected joint snapshot"));
    }
    selected = joint;
  }
  ...
  const AssemblyHierarchyJoint* current = project.find_cross_hierarchy_joint(result.joint);
  ...
  auto drive = detail::resolve_joint_drive(
      *current, AssemblyJointDrive{result.joint, std::move(requested_coordinates)});
  if (drive.has_error())
    return Result<std::size_t>::failure(drive.error());
  if (has_vector_drive && result.requested_coordinates != drive.value().requested_coordinates) {
    return Result<std::size_t>::failure(validation_error(
...
make_motion_result(const Project& source_project, const Project& solved_project,
                   const AssemblyCrossHierarchyMotionGroup& motion_group,
                   const AssemblyHierarchyJoint& selected_joint, double requested_coordinate_deg,
  ...
  return Result<AssemblyCrossHierarchyJointMotionResult>::success(
      AssemblyCrossHierarchyJointMotionResult{
          ...
          std::move(requested_coordinates)});
}

} // namespace

Result<AssemblyCrossHierarchyJointMotionResult>
...
Result<AssemblyCrossHierarchyJointMotionResult>
AssemblyCrossHierarchyJointMotionSolver::move(const Project& project, AssemblyJointDrive drive,
                                              AssemblyRigidBodySolverOptions options) const {
  ...
  for (const AssemblyCrossHierarchyMotionGroup& candidate : graph.value().motion_groups()) {
    if (std::find(candidate.relationships.begin(), candidate.relationships.end(),
                  selected_identity) == candidate.relationships.end()) {
      continue;
    }
    if (motion_group != nullptr) {
      return Result<AssemblyCrossHierarchyJointMotionResult>::failure(validation_error(
          joint_id.value(),
          "selected cross-hierarchy joint must belong to exactly one motion group"));
    }
  ...
  if (motion_group == nullptr) {
    return Result<AssemblyCrossHierarchyJointMotionResult>::failure(validation_error(
        joint_id.value(),
        "selected cross-hierarchy joint must participate in an active motion group"));
  }
  ...
  for (const ComponentTransformAuthority& authority : motion_group->authorities) {
    ...
    if (component.value()->suppression_state() != ComponentSuppressionState::Active) {
      return Result<AssemblyCrossHierarchyJointMotionResult>::failure(
          validation_error(authority.component_instance.value(),
                           "cross-hierarchy motion authority must remain active"));
    }
  ...
  return make_motion_result(project, solved_project, *motion_group, *selected_joint, requested_deg,
                            std::move(selected_drive.value().requested_coordinates),
...
Result<std::size_t> AssemblyCrossHierarchyJointMotionResultApplier::apply(
  ...
  auto selected = validate_selected_joint_snapshot(project, result);
  ...
  auto semantic_freshness = detail::validate_semantic_target_part_snapshots(
      project, detail::semantic_part_documents_from_authority_snapshots(result.authority_snapshots),
  ...
  auto applied = detail::apply_cross_hierarchy_authority_proposals(
      candidate_project, result.proposed_transforms, "assembly.cross_hierarchy_motion_application");
  if (applied.has_error()) {
  ...
  for (const auto& requested : requested_coordinates) {
    auto coordinate_updated = candidate_project.set_cross_hierarchy_joint_coordinate_value(
