...
[[nodiscard]] Error shell_error(const FeatureId& id, std::string message) {
...
[[nodiscard]] Result<ExpectedPlane> expected_plane(const PartDocument& document,
  ...
  if (length(normal) <= kTolerance)
    return Result<ExpectedPlane>::failure(
        shell_error(id, "semantic shell face has degenerate source geometry"));
...
resolve_planar(const PartDocument& document, const ShapeCache& cache, const TopoDS_Shape& target,
  ...
  if (matches.size() != 1U)
    return Result<TopoDS_Face>::failure(
        shell_error(id, matches.empty() ? "semantic shell face no longer exists on target body"
                                        : "semantic shell face is ambiguous on target body"));
...
expected_cylinder(const PartDocument& document, const ShapeCache& cache,
  ...
  if (source == nullptr || source->type() != FeatureType::SubtractiveExtrude)
    return Result<ExpectedCylinder>::failure(
        shell_error(id, "cylindrical shell face producer must be a subtractive extrude"));
  ...
  if (sketch == nullptr || profile == nullptr)
    return Result<ExpectedCylinder>::failure(
        shell_error(id, "cylindrical shell face profile must exist"));
  ...
  if (diameter == nullptr || diameter->type() != ParameterType::Length)
    return Result<ExpectedCylinder>::failure(
        shell_error(id, "cylindrical shell face diameter must resolve to Length"));
...
resolve_cylindrical(const PartDocument& document, const ShapeCache& cache,
  ...
  if (matches.size() != 1U)
    return Result<TopoDS_Face>::failure(shell_error(
        id, matches.empty() ? "semantic cylindrical shell face no longer exists on target body"
                            : "semantic cylindrical shell face is ambiguous on target body"));
  return Result<TopoDS_Face>::success(matches.front());
}
...
Result<GeometryShape> ShellAdapter::apply(const PartDocument& document, const ShellFeature& feature,
                                          const GeometryShape& target,
                                          const ShapeCache& shape_cache,
                                          double thickness_mm) const {
  if (target.empty() || !std::isfinite(thickness_mm) || thickness_mm <= 0.0)
    return Result<GeometryShape>::failure(
        shell_error(feature.id(), "shell target and positive finite thickness are required"));
  try {
    const TopoDS_Shape& target_shape = target.impl_->shape;
    TopTools_ListOfShape closing_faces;
    for (const FaceReference& reference : feature.removed_faces()) {
      ...
      closing_faces.Append(face.value());
    }

    const double offset =
        feature.direction() == ShellDirection::Inward ? -thickness_mm : thickness_mm;
    BRepOffsetAPI_MakeThickSolid builder;
    builder.MakeThickSolidByJoin(target_shape, closing_faces, offset, kTolerance, BRepOffset_Skin,
                                 Standard_False, Standard_False, GeomAbs_Arc, Standard_True);
    if (!builder.IsDone())
      return Result<GeometryShape>::failure(shell_error(
          feature.id(), "OCCT shell did not complete; thickness may exceed local geometry"));
    ...
    if (result.IsNull() || solid_count != 1U || !BRepCheck_Analyzer(result, true).IsValid())
      return Result<GeometryShape>::failure(
          shell_error(feature.id(), "shell produced an invalid or non-manifold solid result"));
    return Result<GeometryShape>::success(
        GeometryShape(std::make_shared<GeometryShape::Impl>(std::move(result))));
  } catch (const Standard_Failure& failure) {
    const char* message = failure.GetMessageString();
    return Result<GeometryShape>::failure(
        shell_error(feature.id(), message == nullptr ? "OCCT shell failed" : std::string(message)));
  }
}
