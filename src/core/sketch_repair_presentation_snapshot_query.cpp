...
[[nodiscard]] json optional_category_to_json(
    const std::optional<SketchRepairDisplayCategory>& category) {
  if (!category.has_value()) return nullptr;
  return std::string(to_string(category.value()));
}

[[nodiscard]] json optional_priority_to_json(
    const std::optional<SketchRepairDisplayPriority>& priority) {
  if (!priority.has_value()) return nullptr;
  return std::string(to_string(priority.value()));
}

[[nodiscard]] SketchRepairPresentationSnapshotQueryCounts compute_counts(
  ...
  for (const auto& entry : snapshot.entries()) {
    ...
    case SketchRepairDisplayCategory::SafeApply: ++safe_apply_count; break;
    case SketchRepairDisplayCategory::RequiresUserChoice: ++requires_user_choice_count; break;
    case SketchRepairDisplayCategory::RequiresParameterValue:
      ++requires_parameter_value_count;
      break;
    case SketchRepairDisplayCategory::UndoEntry: ++undo_entry_count; break;
    }

    switch (entry.priority()) {
    case SketchRepairDisplayPriority::Low: ++low_priority_count; break;
    case SketchRepairDisplayPriority::Normal: ++normal_priority_count; break;
    case SketchRepairDisplayPriority::High: ++high_priority_count; break;
    }
  }

  return SketchRepairPresentationSnapshotQueryCounts(
      safe_apply_count, requires_user_choice_count, requires_parameter_value_count, undo_entry_count,
      low_priority_count, normal_priority_count, high_priority_count);
}

[[nodiscard]] json category_counts_to_json(const SketchRepairPresentationSnapshotQueryCounts& counts) {
...
[[nodiscard]] json priority_counts_to_json(const SketchRepairPresentationSnapshotQueryCounts& counts) {
...
SketchRepairPresentationSnapshotQuery SketchRepairPresentationSnapshotQuery::with_category(
...
SketchRepairPresentationSnapshotQuery SketchRepairPresentationSnapshotQuery::with_priority(
...
void SketchRepairPresentationSnapshotQuery::clear_category_filter() noexcept { category_filter_.reset(); }
...
void SketchRepairPresentationSnapshotQuery::clear_priority_filter() noexcept { priority_filter_.reset(); }
...
bool SketchRepairPresentationSnapshotQuery::latest_only() const noexcept { return latest_only_; }

bool SketchRepairPresentationSnapshotQuery::undoable_only() const noexcept { return undoable_only_; }

bool SketchRepairPresentationSnapshotQuery::matches(
    const SketchRepairPresentationSnapshotEntry& entry) const noexcept {
  if (category_filter_.has_value() && entry.category() != category_filter_.value()) return false;
  if (priority_filter_.has_value() && entry.priority() != priority_filter_.value()) return false;
  if (latest_only_ && !entry.latest()) return false;
  if (undoable_only_ && !entry.undoable()) return false;
  return true;
}

SketchRepairPresentationSnapshotQueryCounts::SketchRepairPresentationSnapshotQueryCounts(
    std::size_t safe_apply_count,
    ...
    : safe_apply_count_(safe_apply_count),
      requires_user_choice_count_(requires_user_choice_count),
      requires_parameter_value_count_(requires_parameter_value_count),
      undo_entry_count_(undo_entry_count),
...
std::size_t SketchRepairPresentationSnapshotQueryCounts::category_count(
  ...
  switch (category) {
  case SketchRepairDisplayCategory::SafeApply: return safe_apply_count_;
  case SketchRepairDisplayCategory::RequiresUserChoice: return requires_user_choice_count_;
  case SketchRepairDisplayCategory::RequiresParameterValue: return requires_parameter_value_count_;
  case SketchRepairDisplayCategory::UndoEntry: return undo_entry_count_;
  }
  return 0U;
}
...
std::size_t SketchRepairPresentationSnapshotQueryCounts::priority_count(
  ...
  switch (priority) {
  case SketchRepairDisplayPriority::Low: return low_priority_count_;
  case SketchRepairDisplayPriority::Normal: return normal_priority_count_;
  case SketchRepairDisplayPriority::High: return high_priority_count_;
  }
  return 0U;
}
...
const SketchRepairPresentationSnapshotQuery& SketchRepairPresentationSnapshotQueryResult::query()
    const noexcept {
  return query_;
}

const SketchRepairPresentationSnapshot& SketchRepairPresentationSnapshotQueryResult::snapshot()
    const noexcept {
  return snapshot_;
}

const SketchRepairPresentationSnapshotQueryCounts& SketchRepairPresentationSnapshotQueryResult::counts()
    const noexcept {
  return counts_;
}
...
SketchRepairPresentationSnapshotQueryResult SketchRepairPresentationSnapshotQueryEngine::execute(
  ...
  for (const auto& entry : snapshot.entries()) {
    if (query.matches(entry)) entries.push_back(entry);
  }
...
Result<std::string> serialize_sketch_repair_presentation_snapshot_query_result_to_json(
  ...
  for (const auto& entry : result.snapshot().entries()) entries.push_back(entry_to_json(entry));
