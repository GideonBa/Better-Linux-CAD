...
added_constraint_ids_for(const SketchRepairTransaction& transaction) {
...
[[nodiscard]] std::vector<SketchConstraintId>
removed_constraint_ids_for(const SketchRepairTransaction& transaction) {
  std::vector<SketchConstraintId> ids;
...
removed_dimension_ids_for(const SketchRepairTransaction& transaction) {
...
std::size_t SketchRepairUndoStackSummaryEntry::index() const noexcept {
  return index_;
}
bool SketchRepairUndoStackSummaryEntry::latest() const noexcept {
  return latest_;
}
SketchRepairTransactionStatus
SketchRepairUndoStackSummaryEntry::transaction_status() const noexcept {
  return transaction_status_;
}
SketchRepairSuggestionAction SketchRepairUndoStackSummaryEntry::action() const noexcept {
  return action_;
}
const std::string& SketchRepairUndoStackSummaryEntry::target() const noexcept {
  return target_;
}
bool SketchRepairUndoStackSummaryEntry::undoable() const noexcept {
  return undoable_;
}
...
std::size_t SketchRepairUndoStackSummary::stack_size() const noexcept {
  return entries_.size();
}
bool SketchRepairUndoStackSummary::empty() const noexcept {
...
SketchRepairUndoStackSummarizer::summarize(const SketchRepairUndoStack& stack) const {
  std::vector<SketchRepairUndoStackSummaryEntry> entries;
  ...
  for (std::size_t index = 0; index < stack.transactions().size(); ++index) {
    ...
    entries.emplace_back(index, index + 1U == stack.transactions().size(), transaction.status(),
                         ...
                         removed_constraint_ids_for(transaction),
                         removed_dimension_ids_for(transaction));
  }
