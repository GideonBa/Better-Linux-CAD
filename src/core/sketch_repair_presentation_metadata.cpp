...
[[nodiscard]] SketchRepairDisplayCategory
category_for_action(SketchRepairSuggestionAction action) noexcept {
  switch (action) {
  case SketchRepairSuggestionAction::AddFixedEndpointConstraint:
  case SketchRepairSuggestionAction::RemoveDuplicateFixedEndpointConstraint:
  case SketchRepairSuggestionAction::RemoveDuplicateDrivingDimension:
...
[[nodiscard]] SketchRepairDisplayPriority
priority_for_action(SketchRepairSuggestionAction action) noexcept {
  switch (action) {
  case SketchRepairSuggestionAction::RemoveConflictingOrientationConstraint:
...
[[nodiscard]] std::string pluralize(std::size_t count, const char* singular, const char* plural) {
...
[[nodiscard]] std::string affected_summary_for(const SketchRepairAffectedCounts& counts) {
  if (counts.empty())
    return "No affected sketch records";
...
std::string_view to_string(SketchRepairDisplayCategory category) noexcept {
  ...
  case SketchRepairDisplayCategory::RequiresParameterValue:
    return "requires_parameter_value";
...
bool SketchRepairAffectedCounts::empty() const noexcept {
  return total() == 0U;
}
...
const std::string& SketchRepairPresentationMetadata::label_id() const noexcept {
  return label_id_;
}
...
SketchRepairPresentationMetadata::affected_counts() const noexcept {
...
SketchRepairPresentationMetadataProvider::metadata_for(SketchRepairSuggestionAction action) const {
...
SketchRepairPresentationMetadata SketchRepairPresentationMetadataProvider::metadata_for(
  ...
  return SketchRepairPresentationMetadata(
      undo_label_id_for_action(entry.action()), SketchRepairDisplayCategory::UndoEntry,
      SketchRepairDisplayPriority::Normal, counts, affected_summary_for(counts));
}
