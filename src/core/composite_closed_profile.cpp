...
Result<CompositeClosedProfile> CompositeClosedProfile::create(
  ...
  if (inner_contours.empty()) {
    return Result<CompositeClosedProfile>::failure(
        validation_error(object_id, "composite closed profile requires at least one inner contour"));
  }

  auto valid_outer = validate_contour(object_id, outer_contour, "outer");
  if (valid_outer.has_error()) return Result<CompositeClosedProfile>::failure(valid_outer.error());

  for (const auto& inner : inner_contours) {
    auto valid_inner = validate_contour(object_id, inner, "inner");
    if (valid_inner.has_error()) return Result<CompositeClosedProfile>::failure(valid_inner.error());
    ...
    for (std::size_t j = i + 1U; j < inner_contours.size(); ++j) {
      for (const auto& line : inner_contours[i]) {
        if (id_in_contour(line, inner_contours[j])) {
          return Result<CompositeClosedProfile>::failure(validation_error(
              object_id, "composite closed profile inner contours must not share line segment ids"));
        }
      }
    }
  }

  return Result<CompositeClosedProfile>::success(CompositeClosedProfile(
      std::move(id), std::move(outer_contour), std::move(inner_contours)));
}

const ProfileId& CompositeClosedProfile::id() const noexcept { return id_; }
const std::vector<SketchEntityId>& CompositeClosedProfile::outer_contour() const noexcept {
  return outer_contour_;
}
const std::vector<std::vector<SketchEntityId>>& CompositeClosedProfile::inner_contours() const noexcept {
...
Result<std::size_t> Sketch::add_profile(CompositeClosedProfile profile) {
  if (find_rectangle_profile(profile.id()) != nullptr || find_circle_profile(profile.id()) != nullptr ||
      find_closed_profile(profile.id()) != nullptr || find_composite_closed_profile(profile.id()) != nullptr) {
...
Result<std::size_t> Sketch::validate_composite_closed_profile(
    const CompositeClosedProfile& profile) const {
  auto outer = ClosedProfile::create(ProfileId(profile.id().value() + ".outer"), profile.outer_contour());
  if (outer.has_error()) return Result<std::size_t>::failure(outer.error());
  auto outer_vertices = validate_closed_profile(outer.value());
  if (outer_vertices.has_error()) return Result<std::size_t>::failure(outer_vertices.error());

  for (std::size_t index = 0; index < profile.inner_contours().size(); ++index) {
    auto inner = ClosedProfile::create(ProfileId(profile.id().value() + ".inner." + std::to_string(index)),
                                      profile.inner_contours()[index]);
    if (inner.has_error()) return Result<std::size_t>::failure(inner.error());
    auto inner_vertices = validate_closed_profile(inner.value());
    if (inner_vertices.has_error()) return Result<std::size_t>::failure(inner_vertices.error());
  }
