...
[[nodiscard]] json composite_inner_contours_to_json(
    const std::vector<std::vector<SketchEntityId>>& contours) {
  json result = json::array();
  for (const auto& contour : contours) result.push_back(sketch_entity_ids_to_json(contour));
  return result;
}

[[nodiscard]] std::vector<std::vector<SketchEntityId>> composite_inner_contours_from_json(
    const json& contours_json) {
  std::vector<std::vector<SketchEntityId>> contours;
  contours.reserve(contours_json.size());
  for (const auto& contour_json : contours_json) {
    contours.push_back(sketch_entity_ids_from_json(contour_json));
  }
  return contours;
}
...
[[nodiscard]] Result<SketchOriginOverrideRecord> sketch_origin_override_from_json(const json& value) {
  return SketchOriginOverrideRecord::create(
      SketchId(value.at("sketch").get<std::string>()), point2_from_json(value.at("local_origin")));
}
...
[[nodiscard]] json construction_relation_to_json(const ConstructionRelation& relation) {
  ...
  case ConstructionRelationType::PointOnGeneratedVertex:
    relation_json["point"] = relation.first_point().value();
    relation_json["generated_vertex"] = semantic_vertex_reference_to_json(relation.generated_vertex().value());
    break;
...
[[nodiscard]] Result<ConstructionRelation> construction_relation_from_json(const json& relation_json) {
  ...
  if (type == "point_on_generated_edge") {
    ...
    if (edge.has_error()) return Result<ConstructionRelation>::failure(edge.error());
  ...
  if (type == "point_on_generated_vertex") {
    ...
    if (vertex.has_error()) return Result<ConstructionRelation>::failure(vertex.error());
...
[[nodiscard]] Result<DerivedWorkplane> derived_workplane_from_json(const json& derived_workplane_json) {
  if (derived_workplane_json.at("kind").get<std::string>() != "feature_face") {
    return Result<DerivedWorkplane>::failure(json_error("only feature-face derived workplanes are supported"));
  }
...
[[nodiscard]] Result<ProjectedSketchPoint> projected_point_from_json(const json& point_json) {
  ...
  if (source == "semantic_vertex") {
    ...
    if (vertex.has_error()) return Result<ProjectedSketchPoint>::failure(vertex.error());
...
[[nodiscard]] Result<ProjectedSketchLine> projected_line_from_json(const json& line_json) {
  ...
  if (source == "semantic_edge") {
    ...
    if (edge.has_error()) return Result<ProjectedSketchLine>::failure(edge.error());
...
[[nodiscard]] Result<ReferenceGeneratedLine> reference_generated_line_from_json(const json& line_json) {
  ...
  if (line_json.contains("direction_constraint")) {
    return ReferenceGeneratedLine::create_with_projected_line_direction(
        id, start, end, SketchConstraintId(line_json.at("direction_constraint").get<std::string>()));
  }
...
[[nodiscard]] Result<SketchConstraint> sketch_constraint_from_json(const json& constraint_json) {
  ...
  if (kind == "coincident_to_projected_point") {
    return SketchConstraint::create_coincident_to_projected_point(id, constrained.value(), reference.value());
  }
  if (kind == "parallel_to_projected_line") {
    return SketchConstraint::create_parallel_to_projected_line(id, constrained.value(), reference.value());
  }
  if (kind == "collinear_with_projected_line") {
    return SketchConstraint::create_collinear_with_projected_line(id, constrained.value(), reference.value());
  }
...
[[nodiscard]] json projected_point_to_json(const ProjectedSketchPoint& point) {
  ...
  if (point.source() == ProjectedSketchPointSource::ConstructionPoint) {
    ...
    point_json["semantic_vertex"] = semantic_vertex_reference_to_json(point.semantic_vertex().value());
  }
  return point_json;
}
...
[[nodiscard]] Result<Sketch> sketch_from_json(const json& sketch_json) {
  ...
  for (const auto& point_json : projected_points) {
    auto point = projected_point_from_json(point_json);
    if (point.has_error()) return Result<Sketch>::failure(point.error());
  ...
  for (const auto& line_json : projected_lines) {
  ...
  for (const auto& line_json : reference_generated_lines) {
  ...
  for (const auto& constraint_json : constraints) {
    auto constraint = sketch_constraint_from_json(constraint_json);
    if (constraint.has_error()) return Result<Sketch>::failure(constraint.error());
  ...
  for (const auto& constraint_json : geometric_constraints) {
  ...
  for (const auto& dimension_json : driving_dimensions) {
    auto dimension = driving_dimension_from_json(dimension_json);
    if (dimension.has_error()) return Result<Sketch>::failure(dimension.error());
  ...
  for (const auto& rectangle_json : rectangle_profiles) {
  ...
  for (const auto& circle_json : circle_profiles) {
  ...
  const json composite_profiles = sketch_json.contains("composite_closed_profiles") ? sketch_json.at("composite_closed_profiles") : json::array();
  for (const auto& profile_json : composite_profiles) {
...
[[nodiscard]] Result<Feature> feature_from_json(const json& feature_json) {
  ...
  if (type == "subtractive_extrude") {
    if (feature_json.at("depth").get<std::string>() != "through_all") {
      return Result<Feature>::failure(json_error("only through_all subtractive extrude depth is supported"));
    }
...
[[nodiscard]] bool sketch_dependencies_available(const PartDocument& document, const Sketch& sketch) {
  ...
  for (const auto& point : sketch.projected_points()) {
    if (point.source() == ProjectedSketchPointSource::ConstructionPoint &&
        document.find_construction_point(point.construction_point()) == nullptr) return false;
  ...
  for (const auto& line : sketch.projected_lines()) {
    if (line.source() == ProjectedSketchLineSource::ConstructionLine &&
        document.find_construction_line(line.construction_line()) == nullptr) return false;
    if (line.source() == ProjectedSketchLineSource::SemanticEdge &&
        document.find_feature(line.semantic_edge().value().source_feature()) == nullptr) return false;
  }
  for (const auto& dimension : sketch.driving_dimensions()) {
    if (document.find_parameter(dimension.parameter()) == nullptr) return false;
  }
  return true;
}
...
Result<std::string> serialize_part_document_to_json(const PartDocument& document) {
  ...
  for (const auto& workplane : document.derived_workplanes()) {
    root["derived_workplanes"].push_back(json{{"id", workplane.id().value()},
                                              {"name", workplane.name()},
                                              {"kind", "feature_face"},
  ...
  for (const auto& sketch : document.sketches()) {
    ...
    for (const auto& profile : sketch.rectangle_profiles()) {
      sketch_json["rectangle_profiles"].push_back(json{{"id", profile.id().value()},
  ...
  for (const auto& status : document.reference_statuses()) root["reference_statuses"].push_back(reference_status_to_json(status));
  root["reference_remaps"] = json::array();
  for (const auto& remap : document.reference_remaps()) root["reference_remaps"].push_back(reference_remap_to_json(remap));
  root["sketch_origin_overrides"] = json::array();
  for (const auto& origin : document.sketch_origin_overrides()) root["sketch_origin_overrides"].push_back(sketch_origin_override_to_json(origin));
...
Result<PartDocument> deserialize_part_document_from_json(std::string_view content) {
  try {
    ...
    for (const auto& parameter_json : root.at("parameters")) {
      ...
      if (parameter.has_error()) return Result<PartDocument>::failure(parameter.error());
    ...
    for (const auto& datum_plane_json : root.at("datum_planes")) {
      ...
      if (datum_plane.has_error()) return Result<PartDocument>::failure(datum_plane.error());
    ...
    for (const auto& point_json : construction_point_array) {
      ...
      if (point.has_error()) return Result<PartDocument>::failure(point.error());
    ...
    while (remaining > 0U) {
      ...
      for (std::size_t index = 0; index < sketch_array.size(); ++index) {
        ...
        if (!sketch_dependencies_available(document.value(), sketch.value())) continue;
      ...
      for (std::size_t index = 0; index < feature_array.size(); ++index) {
        ...
        if (feature.value().type() == FeatureType::AdditiveExtrude &&
          ...
          return Result<PartDocument>::failure(json_error("additive extrude length parameter must exist in part document"));
        }
        if (feature.value().type() == FeatureType::SubtractiveExtrude &&
            document.value().find_feature(feature.value().target_feature()) == nullptr) continue;
      ...
      for (std::size_t index = 0; index < construction_line_array.size(); ++index) {
        ...
        if (added.has_error()) {
          const auto message = added.error().message();
          if (message == "line parallel to line through point references must exist in part document" ||
    ...
    for (const auto& status_json : reference_status_array) {
      auto status = reference_status_from_json(status_json);
      if (status.has_error()) return Result<PartDocument>::failure(status.error());
    ...
    for (const auto& remap_json : reference_remap_array) {
      auto remap = reference_remap_from_json(remap_json);
      if (remap.has_error()) return Result<PartDocument>::failure(remap.error());
    ...
    for (const auto& origin_json : origin_override_array) {
      auto origin = sketch_origin_override_from_json(origin_json);
      if (origin.has_error()) return Result<PartDocument>::failure(origin.error());
