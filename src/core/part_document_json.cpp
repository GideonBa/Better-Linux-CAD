...
[[nodiscard]] Result<Sketch> sketch_from_json(const json& sketch_json) {
  ...
  const json projected_points =
      sketch_json.contains("projected_points") ? sketch_json.at("projected_points") : json::array();
  ...
  const json projected_lines =
      sketch_json.contains("projected_lines") ? sketch_json.at("projected_lines") : json::array();
  ...
  const json rectangle_profiles = sketch_json.contains("rectangle_profiles")
                                      ? sketch_json.at("rectangle_profiles")
                                      : json::array();
  ...
  const json circle_profiles =
      sketch_json.contains("circle_profiles") ? sketch_json.at("circle_profiles") : json::array();
  ...
  try {
    ...
    while (remaining_construction_lines > 0U) {
      ...
      for (std::size_t index = 0; index < construction_line_array.size(); ++index) {
        ...
        --remaining_construction_lines;
        progress = true;
      }
      if (!progress) {
        return Result<PartDocument>::failure(
            json_error("could not resolve construction line json dependencies"));
      }
    }
