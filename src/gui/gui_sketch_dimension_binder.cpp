...
constexpr std::array<DimensionActionDescriptor, 9U> kDimensionActions{{
...
void append_diagnostic(MainWindow& window, const Error& error) {
...
Result<SketchTopology> active_topology(const MainWindow& window) {
...
std::optional<std::string> entity_id_from_semantic(const SketchTopology& topology,
                                                   std::string semantic) {
  ...
  if (std::string_view(semantic).starts_with("entity/")) {
    if (topology.find_entity(semantic) != nullptr) return semantic;
  } else if (std::string_view(semantic).starts_with("profile/")) {
    if (topology.find_entity(semantic) != nullptr) return semantic;
...
std::optional<SketchPointId> point_id_from_candidate(
...
Result<std::vector<SketchConstraintIntentTarget>> selected_targets(
  ...
  for (const auto& selection : window.session().selection().entries()) {
    ...
    const auto entity_id = entity_id_from_semantic(topology, selection.semantic_id);
    if (!entity_id)
    ...
    if (!selection.candidate_id.empty()) {
      const auto point_id = point_id_from_candidate(topology, *entity, selection.candidate_id);
      if (point_id) {
...
std::optional<SketchDimensionId> selected_dimension(const MainWindow& window,
...
void normalize_target_order(const SketchTopology& topology, SketchDimensionKind kind,
                            std::vector<SketchConstraintIntentTarget>& targets) {
...
SketchDimensionId next_dimension_id(const SketchDimensionCatalog& catalog,
                                    SketchDimensionKind kind) {
...
std::optional<ParameterId> select_parameter(MainWindow& window, SketchDimensionKind kind) {
  const auto parameters = GuiSketchDimensionController::compatible_parameters(
      *window.session().part_document(), kind);
  if (parameters.empty()) {
    append_diagnostic(window, Error::validation(
...
Result<std::size_t> execute_add(MainWindow& window, SketchDimensionKind kind,
                                bool reference_mode) {
  ...
  normalize_target_order(topology.value(), kind, targets.value());
  ...
  if (mode == SketchDimensionMode::Driving) {
    parameter = select_parameter(window, kind);
    if (!parameter) return Result<std::size_t>::success(0U);
  }
  auto intent = SketchDimensionIntent::create(next_dimension_id(catalog.value(), kind), kind,
                                               mode, std::move(targets.value()), parameter);
...
Result<std::size_t> execute_edit(MainWindow& window) {
  if (!window.active_sketch() || window.session().part_document() == nullptr)
    return Result<std::size_t>::failure(Error::validation(
        "sketch_dimension_gui", "dimension edit requires an active Sketch"));
  const auto selected = selected_dimension(window, *window.active_sketch());
  if (!selected)
    return Result<std::size_t>::failure(Error::validation(
...
Result<std::size_t> execute_rebind(MainWindow& window) {
  ...
  auto parameter = select_parameter(window, dimension->kind());
  if (!parameter) return Result<std::size_t>::success(0U);
  return GuiSketchDimensionController::rebind_parameter(
      window.session(), *window.active_sketch(), *selected, *parameter);
}

Result<std::size_t> execute_toggle_mode(MainWindow& window) {
  if (!window.active_sketch())
    return Result<std::size_t>::failure(Error::validation(
...
public:
  explicit SketchDimensionBinder(MainWindow& window) : QObject(&window), window_(window) {
    ...
    QMenu* root = sketch_menu(window_);
    ...
    for (const auto& descriptor : kDimensionActions) {
      ...
      connect(action, &QAction::triggered, this, [this, kind = descriptor.kind] {
        run([this, kind] { return execute_add(window_, kind, reference_mode_->isChecked()); });
      });
    ...
    connect(edit_action_, &QAction::triggered, this,
            [this] { run([this] { return execute_edit(window_); }); });
    connect(rebind_action_, &QAction::triggered, this,
            [this] { run([this] { return execute_rebind(window_); }); });
    connect(mode_action_, &QAction::triggered, this,
            [this] { run([this] { return execute_toggle_mode(window_); }); });
    if (menu_ != nullptr) {
  ...
  void refresh() {
    ...
    const bool dimension_selected =
        active && selected_dimension(window_, *window_.active_sketch()).has_value();
...
private:
  template <typename Operation> void run(Operation operation) {
    ...
    if (result.has_error()) append_diagnostic(window_, result.error());
...
void install_sketch_dimension_binder(MainWindow& window) {
  if (window.findChild<QObject*>(QStringLiteral("blcad.sketch.dimension_binder")) != nullptr)
    return;
  (void)new SketchDimensionBinder(window);
}

void refresh_sketch_dimension_actions(MainWindow& window) {
  if (auto* binder =
          window.findChild<SketchDimensionBinder*>(QStringLiteral("blcad.sketch.dimension_binder")))
    binder->refresh();
}
