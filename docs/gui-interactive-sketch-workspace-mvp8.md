...
On success the controller restores the previous GUI workspace and semantic selection. `MainWindow` restores the captured viewport Eye, Target, Up, Scale, projection, plane-camera publication, and selection-filter mask, removes the crosshair, and clears the active Sketch workspace state.

For example, entering `sketch.bracket` from the Inspect workspace with a datum selected clears the working selection, edits normal to the Sketch plane, and returns to Inspect with the same datum selection when Finish succeeds.
...
The `Sketch -> Repeat last Sketch command` action exposes this authority. The initial line integration reopens the numeric HUD for `sketch.line`. Repeat state is cleared when the user leaves the Sketch workspace.
...
This closes the Block-106 requirement that normal-to-plane Sketch editing must return the user to the previous camera instead of forcing a generic isometric fallback.

## Transaction and persistence boundaries

Block 106 introduces no new JSON field and no schema migration.

Persistent geometry mutation still goes through `GuiDocumentSession::commit_part_transaction(...)`, which validates and recomputes the candidate before publishing one canonical snapshot/history entry. Preview state, HUD text, command repeat, hover, cursor coordinates, snap text, focus, workspace bookmarks, camera bookmarks, and Sketch status are transient only.
...
They cover context capture/restore, command/browser surface order, the selection mask, status publication, staged `Esc`, repeat, handle/drag-candidate transience, generic task backtracking, and camera bookmark restoration. Existing Block-99 tests continue to prove normal-to-plane Sketch entry and the shared workbench mutation path.
