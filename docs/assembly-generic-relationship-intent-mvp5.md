...
Block 40 joint target compatibility and the oriented `Frame` contract are implemented in `docs/assembly-joint-target-compatibility-mvp5.md`. Blocks 41–43 typed coordinate slots, additive compatible JSON, and shared vector drives are implemented in their canonical joint documents. Blocks 44–46 Prismatic, Cylindrical, and Planar are implemented in their canonical documents. The next technical step is Block 47 Ball/Spherical.
