...
Block 40 joint target compatibility and the oriented `Frame` contract are implemented in `docs/assembly-joint-target-compatibility-mvp5.md`. Blocks 41–42 typed coordinate slots and additive compatible JSON are implemented in `docs/assembly-joint-coordinate-model-mvp5.md` and `docs/assembly-joint-coordinate-json-mvp5.md`. The next technical step is Block 43: vector joint drives, holding semantics, freshness, and atomic application.
