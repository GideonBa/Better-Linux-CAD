...
After the command-label seed, each entry includes stable presentation fields generated by `SketchRepairCommandLabeler`:
...
After the presentation-metadata seed, each entry also includes machine-readable presentation metadata generated by `SketchRepairPresentationMetadataProvider`:
...
These labels and metadata are read-only UI/CLI presentation data. They do not affect undo behavior, transaction ordering, model validation, or `.blcad.json` persistence.
...
- debug JSON containing presentation metadata fields
...
Presentation labels are implemented separately in `docs/sketch-repair-command-labels-mvp.md`; presentation metadata is implemented separately in `docs/sketch-repair-presentation-metadata-mvp.md`.
