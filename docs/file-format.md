...
Status: implemented seeds exist for single-part `.blcad.json`, assembly-parameter JSON, embedded project JSON, component instances with explicit placement/state updates, and solver-independent Mate/Concentric/Distance assembly constraint records. Multi-body part records, body transforms, body booleans, solver state, and path-feature records remain future blocks.
...
- constraint graph, DOF, semantic target geometry resolution, assembly-level geometry instancing, and assembly-level STEP export are not implemented in the current block

See `docs/assembly-constraint-model-intent-mvp5.md` for the exact implemented record and compatibility boundary.
...
- serialization stores model intent only; it does not serialize OCCT shapes, `ShapeCache` contents, solved assembly transforms, or DOF state.
- `derived_workplanes` with `top/bottom/right/left/front/back` faces are persisted.

See `docs/json-serialization-mvp1.md`, `docs/json-file-workflow-mvp1.md`, `docs/assembly-parameters-mvp4.md`, `docs/project-container-mvp4.md`, `docs/component-instance-mvp5.md`, and `docs/assembly-constraint-model-intent-mvp5.md`.
...
12. Add no separate persisted graph state for the next read-only constraint graph seed because adjacency and connected groups are regenerable from active constraint records.
13. Add regenerable assembly solver/cache records only after semantic target resolution and a rigid-body solver exist, and persist only data that is not safely derivable.
14. Add project-level `resources` (materials, standard parts) referenced by engineering modules (`docs/engineering-modules.md`).
15. Optionally add an out-of-band `cached_shapes` store, kept clearly separate from model intent.
...
- read-only constraint graph connectivity should be regenerated from component and constraint model intent rather than serialized as a second source of truth.
