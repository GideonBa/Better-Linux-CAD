...
Status: core serialization for `PartDocument` model intent, including file-level helpers and derived workplanes.
...
The derived-workplane seed model is:

```text
examples/top_face_cut.blcad.json
```

The second file describes a sketch placed on `workplane.base_top`, which is derived from `feature.base_extrude.top`.
...
This does not store an OCCT face ID. It says that the workplane is derived from the semantic top face generated by the additive extrude feature.

For now, only `kind = "feature_face"` and `face = "top"` are supported.
...
```json
{
  "id": "sketch.top_hole",
  "name": "Sketch_TopHole",
  "workplane": "workplane.base_top",
  "rectangle_profiles": [],
  "circle_profiles": [
    {
      "id": "profile.top_hole",
      "center": { "x": 0.0, "y": 0.0 },
      "diameter_parameter": "part.top_hole_diameter"
    }
  ]
}
```
...
```json
{
  "id": "feature.top_hole_cut",
  "name": "TopHoleCut",
  "type": "subtractive_extrude",
  "input_sketch": "sketch.top_hole",
  "direction": "+Z",
...
- derived workplanes round-trip through JSON
...
- arbitrary semantic faces beyond the top face of a simple additive extrude
