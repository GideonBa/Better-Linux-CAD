...
- ordered BodyTransform intent and canonical SketchOwnership associations
...
are regenerated by the Block-55 Geometry executor. The persistent contract is documented in
`docs/part-body-boolean-mvp6.md`; execution is documented in
`docs/part-body-boolean-geometry-mvp6.md`.

Block 56 adds optional-on-read, always-emitted `body_transforms` and `sketch_ownerships`. Transform
records persist stack order, coordinate/rotation reference identity, direct numeric intent, and
associative flags; they never store transformed OCCT shapes or resolved workplane frames. The
canonical contract is `docs/part-body-transform-ownership-mvp6.md`.
