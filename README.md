...
The next technical step should add the first sketch solver diagnostics seed.

1. Add a lightweight `SketchConstraintDiagnostic` record or report type for sketches.
2. Detect simple under-constrained cases: unconstrained line endpoints, free spline control points, and profiles without driving dimensions.
3. Detect simple over-constrained conflicts for the current deterministic subset: duplicate horizontal/vertical contradictions, impossible fixed endpoint pairs, and duplicate driving dimensions on the same target.
4. Emit diagnostics without rewriting geometry or solving constraints.
5. Serialize optional diagnostic snapshots only as debug/output artifacts, not as model intent.
6. Add core tests for under-constrained and over-constrained reports.
7. Keep full constraint solving, automatic repair, GUI highlighting, degrees-of-freedom counting, spline-handle solving, and parametric solve iteration deferred.
