...
The next technical step should add the first sketch repair suggestion seed.

1. Add a `SketchRepairSuggestion` report type linked to diagnostic kinds and targets.
2. Suggest fixed endpoint constraints for unconstrained line endpoints.
3. Suggest removing one side of a horizontal/vertical contradiction.
4. Suggest removing duplicate fixed endpoint constraints and duplicate driving dimensions.
5. Suggest adding a driving dimension to profile sketches with no dimension intent.
6. Serialize suggestion reports as debug/output artifacts only, not as `.blcad.json` model intent.
7. Add core tests that suggestions are deterministic and reference the originating diagnostics.
8. Keep automatic application, GUI editing workflows, full solve iteration, exact DOF counting, spline-handle solving, and model rewriting deferred.
