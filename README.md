...
- `docs/json-serialization-mvp1.md`: JSON serialization of model intent, including reference-generated helper lines and reference recovery metadata
...
Reference-generated helper lines are now first-class sketch model intent. `ReferenceGeneratedLine` stores the endpoint constraints and an optional projected-line direction constraint. Sketch JSON persists them in `reference_generated_lines`, `PartDocument` validates the endpoint/direction constraints and adds helper-line dependency graph nodes, and `ReferenceGeneratedProfileResolver` resolves those records into deterministic local helper lines. `GeometryRecomputeExecutor` consumes reference-generated helper-line closed profiles directly for additive extrudes and subtractive through-all cuts.
...
Still not implemented: arcs, splines, trimmed curves, automatic region detection, multiple contours in one feature, inner holes in one profile, mixed explicit-line/reference-generated-line closed profiles, a full sketch constraint solver, automatic profile topology from projected references without helper constraints, GUI reference-remapping dialogs, automatic topology matching after arbitrary edits, GUI sketch editing, arbitrary non-planar sketch geometry, arbitrary generated edge/vertex topology evaluation, analytic generated-surface construction references, revolve, revolve cut, 3D guide curves, loft, sweep, boundary surfaces, surface stitching, or closed-shell-to-solid conversion.
...
The next technical step should add the first explicit sketch constraints and driving dimensions for deterministic line-segment geometry without adding a full sketch solver yet.

1. Add model-intent records for fixed, horizontal, vertical, parallel, perpendicular, and equal-length constraints on explicit line segments.
2. Add first driving dimension records for horizontal, vertical, aligned, and point-to-point distances backed by existing length parameters.
3. Validate dimensions against existing sketch entities and parameters.
4. Add JSON roundtrip tests for geometric constraints and driving dimensions.
5. Add invalidation graph edges from dimension parameters to sketches and dependent features.
6. Add geometry tests proving dimension parameter changes alter closed-profile recompute where deterministic.
7. Keep automatic solving, over/under-constrained diagnosis, GUI constraint display, automatic region detection, arcs, splines, 3D sketches, sweep, loft, surface stitching, and closed-shell-to-solid conversion deferred.
