...
The next technical step should add a small in-memory sketch repair undo stack seed.

1. Add a `SketchRepairUndoStack` or equivalent in-memory helper that stores applied `SketchRepairTransaction` records in application order.
2. Support pushing only applied and undoable repair transactions.
3. Support undoing the most recent transaction first by delegating to `SketchRepairTransactionExecutor::undo`.
4. Return a structured stack result with the transaction status and remaining stack size.
5. Keep redo, persistent transaction journals, multi-sketch stacks, GUI integration, parameter-creating repairs, full solve iteration, exact DOF counting, and arbitrary model rewriting deferred.
6. Add core tests that push multiple safe repair transactions and undo them in strict LIFO order.
