...
- `docs/construction-geometry-mvp.md`: implemented explicit and relation-driven construction-geometry seed
...
The current core skeleton covers `Quantity`, typed IDs, `Error`, `Result`, `Parameter`, `PartDocument`, `DatumPlane`, `DerivedWorkplane`, `ConstructionPoint`, `ConstructionLine`, `ConstructionPlane`, `ConstructionRelation`, `Sketch`, `LineSegment`, `RectangleProfile`, `CircleProfile`, and `ClosedProfile`, as well as `Feature`, `AdditiveExtrude`, `SubtractiveExtrude`, `DependencyGraph`, `InvalidationState`, and `RecomputePlan`. `PartDocument` validates workplane, profile, construction-geometry, construction-relation, and feature references, creates graph nodes and graph edges from them, marks affected nodes after a parameter change, and derives an ordered recompute plan from that state.
...
The construction-geometry path now has explicit construction points, explicit construction lines, explicit construction planes, and first relation-driven construction definitions. Implemented relation types are `PlaneOffsetFromPlane`, `LineThroughTwoPoints`, and `PlaneThroughThreePoints`. Construction planes can be used as sketch workplanes. Construction relations create dependency graph edges from referenced parameters, construction points, and source workplanes to the relation-driven construction object. These dependencies are still model-intent and invalidation/recompute hooks; there is no GUI editing or full sketch constraint solver yet.

The optional geometry build resolves sketch workplanes before executing subtractive cuts and before executing line-based closed-profile geometry. `WorkplaneResolver` can resolve `datum.xy`, `feature.base_extrude.top`, `feature.base_extrude.bottom`, `feature.base_extrude.right`, `feature.base_extrude.left`, `feature.base_extrude.front`, `feature.base_extrude.back`, explicit construction planes, offset construction-plane relations, and three-point construction-plane relations. Top and bottom cuts are vertical through-all cuts. Right and left face cuts are X-axis through-all cuts. Front and back face cuts are Y-axis through-all cuts. All currently supported generated-face workplanes carry rectangular bounds derived from the source rectangle sketch and extrude thickness. Construction planes are currently treated as unbounded reference planes.
...
The next technical step should generalize the construction-relation seed into a semantic reference system for generated edges, generated vertices, and chained construction relations, while keeping raw OCCT topology out of saved model intent.
...
4. Add `PlaneParallelToPlaneThroughPoint` and `LineParallelToLineThroughPoint` relation definitions.
5. Add dependency graph tests for chained construction relations.
6. Add JSON roundtrip tests for chained relations and semantic generated references.
7. Extend `WorkplaneResolver` only for relations that can be evaluated deterministically from existing model intent.
8. Keep explicit construction geometry and the current relation-driven path as stable fallback paths.
9. Do not add GUI manipulators yet.
10. Do not add a sketch constraint solver yet.
11. Do not add 3D sketch splines, sweep, loft, surface stitching, or closed-shell-to-solid conversion yet.

The completed explicit and relation-driven construction-geometry seed is documented in `docs/construction-geometry-mvp.md`.
