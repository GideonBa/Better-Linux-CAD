...
- `docs/json-serialization-mvp1.md`: JSON serialization of model intent, including arc records, sketch dimensions, generated-region selections, composite profiles, reference-generated helper lines, and reference recovery metadata
...
`ArcSegment` stores a first explicit circular arc through three sketch-local points. `ArcClosedProfile` stores ordered explicit line/arc curve IDs and validates deterministic end-to-start closure plus first line/line, line/arc, and arc/arc self-intersection cases. `SketchTrimExtendOperation` stores explicit trim/extend metadata for line/arc endpoints, but it does not yet solve or rewrite curve geometry. These records now serialize and deserialize through `.blcad.json`.
...
Export the closed-profile, composite-profile, and construction-plane models:
...
The next technical step should add the first spline and tangent-continuity sketch profile seed.

1. Add a first `SplineSegment` model-intent record, preferably a cubic Bezier or small control-point spline seed.
2. Add ordered line/arc/spline closed-profile support without storing solver state.
3. Add first tangent-continuity metadata between adjacent line, arc, and spline entities.
4. Serialize spline segments and tangent metadata to JSON.
5. Add OCCT edge construction for the first spline representation.
6. Add additive and subtractive recompute tests for one simple line/arc/spline profile.
7. Keep full spline editing, NURBS weights, automatic fillets, tangent constraint solving, GUI curve editing, 3D sketches, sweep, loft, surface stitching, and closed-shell-to-solid conversion deferred.
