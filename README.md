...
`CompositeClosedProfile` adds the first explicit multi-contour profile path. It stores one ordered outer contour plus ordered inner contours. The geometry layer validates strict inner-contour containment, maps all contours through the resolved sketch workplane, and creates OCCT faces with inner wires for additive extrudes and subtractive through-all cuts.
...
The next technical step should add the first arc and trim/extend seed for sketch profiles.

1. Add a sketch arc entity with stable IDs, center/radius or three-point construction, and deterministic JSON intent.
2. Add contour validation that supports ordered line/arc loops without converting arcs to raw tessellation as model intent.
3. Extend the geometry adapter to build OCCT wires containing line and circular-arc edges.
4. Add trim/extend metadata for line/arc endpoints as explicit model intent, not as GUI-only state.
5. Add additive extrude and subtractive cut tests for one profile containing a circular arc.
6. Keep splines, tangent constraint solving, automatic fillets, full GUI editing, 3D sketches, sweep, loft, surface stitching, and closed-shell-to-solid conversion deferred.
