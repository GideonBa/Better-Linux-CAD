...
The next technical step should turn the first reference-driven helper geometry into reproducible profile input for feature operations without adding a full sketch solver or automatic region detector yet.

1. Add model-intent records for reference-generated helper line entities derived from projected endpoint constraints.
2. Validate helper-generated line endpoints and projected-line direction consistency before profile construction.
3. Allow a `ClosedProfile` to explicitly consume deterministic reference-generated helper line IDs where all helpers are fully resolved.
4. Add JSON roundtrip tests for reference-generated helper entity records.
5. Add geometry tests for an additive extrude and a subtractive cut whose closed profile uses deterministic projected-reference helper lines.
6. Add invalidation tests proving projected source changes dirty helper-generated profiles and dependent features.
7. Keep full sketch solving, automatic region detection, GUI editing, arbitrary generated topology, 3D sketches, sweep, loft, surface stitching, and closed-shell-to-solid conversion deferred.
