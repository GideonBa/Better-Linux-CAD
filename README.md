...
- `docs/json-serialization-mvp1.md`: JSON serialization of model intent, including sketch dimensions, reference-generated helper lines, and reference recovery metadata
...
The next technical step should add the first deterministic automatic profile region detection seed.

1. Add a sketch-region finder for unordered explicit line segments on one sketch.
2. Support simple single-contour loops with no branches, duplicate edges, gaps, or self-intersections.
3. Allow resolved reference-generated helper lines and dimension-driven explicit lines to participate only after they resolve to local line segments.
4. Create stable generated `ClosedProfile` candidates with deterministic IDs.
5. Add validation errors for ambiguous, branched, duplicate, or self-intersecting regions.
6. Add JSON metadata only for user-selected/generated-region intent, not raw solver state.
7. Add geometry tests for additive and subtractive recompute from detected simple regions.
8. Keep full sketch solving, multiple regions, inner loops, trim/extend, arcs, splines, GUI selection, 3D sketches, sweep, loft, surface stitching, and closed-shell-to-solid conversion deferred.
