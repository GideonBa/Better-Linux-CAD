...
The next technical step should add a sketch repair command transaction and undo seed.

1. Add a `SketchRepairTransaction` or equivalent record that groups one or more applied `SketchRepairCommand` results.
2. Record enough before/after metadata to undo the safe command subset without serializing full sketch snapshots.
3. Support undo for added fixed endpoint constraints, removed duplicate fixed endpoint constraints, and removed duplicate driving dimensions.
4. Require explicit caller intent for both apply and undo; transactions must never run automatically during diagnostics or suggestion generation.
5. Add core tests that apply plus undo restores the exact affected sketch records for the safe subset.
6. Keep GUI undo stacks, persistent transaction journals, multi-sketch transactions, parameter-creating repairs, full solve iteration, exact DOF counting, and arbitrary model rewriting deferred.
