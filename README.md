...
Load the generated-reference example model:
...
The next technical step should project evaluated semantic generated edges, generated vertices, construction points, and construction lines into sketch-local reference geometry so sketches can consume projected references without storing raw OCCT topology and without adding a full sketch constraint solver yet.

1. Add sketch-level reference entity IDs for projected construction points and projected construction lines.
2. Add model-intent references from sketch reference entities to `ConstructionPointId`, `ConstructionLineId`, `SemanticVertexReference`, and `SemanticEdgeReference`.
3. Project resolved 3D construction points and construction lines into a resolved sketch workplane when the reference lies on that workplane.
4. Validate out-of-plane projections with explicit errors.
5. Add JSON roundtrip tests for projected sketch reference entities.
6. Add geometry tests using projected references as closed-profile helper geometry where deterministic.
