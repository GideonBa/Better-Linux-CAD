...
The current state is a deliberately small MVP-1 vertical slice plus the first executable seed of MVP 2. It contains base types for parameters, length quantities, error handling, a pure `PartDocument` model, datum planes, derived workplanes, sketches, feature-intent data models, a dependency graph, invalidation state, recompute planning, JSON model persistence, optional OCCT geometry execution, top/bottom/right/left workplane resolution, bounded face validation, incremental derived-workplane recompute, and STEP export. There is no GUI yet.
...
MVP 2 has started with a minimal semantic-face path. `SemanticFaceReference` can point to `feature.base_extrude.top`, `feature.base_extrude.bottom`, `feature.base_extrude.right`, and `feature.base_extrude.left`. `DerivedWorkplane` can expose those semantic faces as sketch workplanes, and a sketch can use a derived workplane as its workplane reference. The dependency graph represents this as `feature.base_extrude -> workplane -> sketch -> cut_feature`.

The optional geometry build resolves sketch workplanes before executing subtractive cuts. `WorkplaneResolver` can resolve `datum.xy`, `feature.base_extrude.top`, `feature.base_extrude.bottom`, `feature.base_extrude.right`, and `feature.base_extrude.left`. Top and bottom cuts are vertical through-all cuts. Right and left face cuts are X-axis through-all cuts using the resolved workplane normal. All currently supported generated-face workplanes carry rectangular bounds derived from the source rectangle sketch and extrude thickness.
...
The next technical step should add one remaining Y-side semantic reference, still without broadening into arbitrary topology.

1. Add `SemanticFace::Front` for the `+Y` face of a simple `AdditiveExtrude`.
2. Allow `DerivedWorkplane` to reference `feature.base_extrude.front`.
3. Resolve the front-face workplane with a clear origin, axes, normal, and rectangular bounds.
4. Define local front-face coordinates consistently, for example local width along X and local height along Z.
5. Add JSON roundtrip coverage for the front-face derived workplane.
6. Add geometry tests that place a sketch on the front face and execute a through-all Y-axis circular cut.
7. Keep support limited to top, bottom, right, left, and front faces of a simple `AdditiveExtrude`.
8. Do not add the back face yet.
